...
| `START_TIME` | 首次执行续期的基准时间（北京时间，24小时制），格式：`YYYY-MM-DD HH:mm`（如 `2026-08-10 13:30` 代表下午1点半）。脚本将以此时间为起点每天顺延 5 分钟。留空则使用默认基准。通常为当天的广告冷却后时间。 | 选填 |
...
** 🔄 自动续期：代码会自动计算 24 小时冷却期，并每天自动以设定的 START_TIME 为基础顺延 5 分钟执行。在未到达当天特定时间点前，绝对不会触发续期请求。

如果修改触发器（Cron）为10分钟（*/10 * * * *），则隧道监控和自动续期间隔都为10分钟，以此类推
